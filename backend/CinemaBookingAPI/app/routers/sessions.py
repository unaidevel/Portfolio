...
async def update_session_by_id(session_id:UUID, session_update: SessionUpdate, session: SessionDep, current_user: Annotated[str, Depends(admin_only)]):
...
async def delete_session(session_id: UUID, session: SessionDep, current_user: Annotated[str, Depends(admin_only)]):
    specific_session = session.get(Session, session_id)
